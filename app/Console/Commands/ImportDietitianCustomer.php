...
class ImportDietitianCustomer extends Command {
     ...
     * @var string
     */
    protected $signature = "import-dietitian-customer {--file=''} {--usertype=''}";
...
//        try {
        ...
        Excel::load($options['file'])->each(function (Collection $csvLine) use($admin, &$failed, &$successfulCount, &$failedCount) {
            ...
            $username = trim($csvLine->get('username'));
            $designation = trim($csvLine->get('designation'));
            $email = (trim($csvLine->get('email')) != '') ? trim($csvLine->get('email')) : $username;
...
//Log::info("Imported " . $username . " successfully.");
                ...
                $response = $this->memberRepository->importMemberDataOriginal(['username' => $username]);
...
//Log::error("\nError while importing customers for " . $username . ". " . $response['message']);
                } else {
