...
class OfferUpdateRequest extends Request {
    ...
    public function rules() {
        ...
        $id = $this->offers->id;
        return [
            'offer_title' => 'required|regex:/^(?![\W\s]+$).+$/m|min:2|max:50|unique:offers,offer_title,' . $id,
    ...
    public function messages() {
        return [
            ...
            'offer_detail_page_url.min' => trans('admin::messages.error-minlength-number', ['name' => trans('admin::controller/offers.offer_detail-page-url'), 'number' => '2']),
            'offer_detail_page_url.alphaSpaces' => trans('admin::messages.error-alpha-spaces', ['name' => trans('admin::controller/offers.offer_detail-page-url')]),
    ...
    public function authorize() {
        ...
        if ($is_edit == 1 || (!empty($own_edit) && ($this->offers->created_by == Auth::guard('admin')->user()->id))) {
            return true;
        } else {
            abort(403);
        }
