...
/**
 * The class for handling validation requests from DietPlanDetailController::Update()
 *
 *
...
class DietPlanDetailUpdateRequest extends Request {
...
//            'status.numeric' => trans('admin::messages.error-numeric-id', ['name' => trans('admin::controller/diet-plan-detail.status')]),
    ...
    public function authorize() {
        ...
        if ($is_edit == 1 || (!empty($own_edit) && ($this->diet_plan_detail->created_by == Auth::guard('admin')->user()->id))) {
            return true;
        } else {
            abort(403);
        }
