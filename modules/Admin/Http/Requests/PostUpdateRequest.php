<?php
/**
 * The class for handling validation requests from PostController::update()
 * 
 * 
...
class PostUpdateRequest extends Request
{
    ...
    public function rules()
    {
        $this->sanitize();
        return [
            'name' => 'required|max:200|unique:posts,name,' . $this->posts->id, //alphaSpaces
    ...
    public function authorize()
    {
        ...
        if ($is_edit == 1 || (!empty($own_edit) && ($this->posts->created_by == Auth::guard('admin')->user()->id))) {
            return true;
        } else {
            abort(403);
        }
