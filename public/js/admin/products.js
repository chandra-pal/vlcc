siteObjJs.admin.productsJs = function () {
...
// Initialize all the page-specific event listeners here.
    ...
    var initializeListener = function () {
        ...
        $('body').on("click", ".remove-image", function () {
            ...
            bootbox.confirm({
                ...
                message: siteObjJs.admin.productsJs.confirmRemoveProductImage,
        ...
        $('body').on('click', '.delete', function (e) {
            ...
            function handleGroupAction(grid) {
                var token = $('meta[name="csrf-token"]').attr('content');
                var actionUrl = 'products/' + productsId;
                jQuery.ajax({
                    url: actionUrl,
                    cache: false,
                    data: {
    ...
    var fetchDataForEdit = function () {
        $('.portlet-body').on('click', '.edit-form-link', function () {
            ...
            var actionUrl = 'products/' + cat_id + '/edit';
            $.ajax({
                url: actionUrl,
                cache: false,
                dataType: "json",
                ...
                success: function (data)
                {
                    ...
                    siteObjJs.validation.formValidateInit('#edit-products', handleAjaxRequest);
                },
    ...
    var handleAjaxRequest = function () {
        ...
        if (image) {
            if (image.size > 2097152) {
                var error = siteObjJs.admin.productsJs.maxFileSize;
                $('#' + formid + ' #file-error').text(error);
                return false;
            }
            var ext = $('#' + formid + ' #thefile').val().split('.').pop().toLowerCase();
            if ($.inArray(ext, ['gif', 'png', 'jpg', 'jpeg']) == -1) {
                var error = siteObjJs.admin.productsJs.mimes;
                $('#' + formid + ' #file-error').text(error);
                return false;
            }
        ...
        grid.init({
            ...
            dataTable: {
                ...
                "columns": [
                    ...
                    {data: 'product_detail_page_url', name: 'product_detail_page_url'},
    ...
    return {
        ...
        init: function () {
            ...
            siteObjJs.validation.formValidateInit('#create-products', handleAjaxRequest);
        }
