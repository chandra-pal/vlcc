siteObjJs.admin.commonJs = function () {
    ...
    var boxExpandBtnClick = function () {
        $('body').on('click', '.btn-expand-form', function () {
            ...
            $('.add-form-main form :input:visible:enabled:first').not(".skip").focus();
        });
        $('body').on('click', '.btn-collapse-form', function () {
            $(this).closest('form input:not(.skip-select)').trigger('reset');
        ...
        $('.togglelable').click(function (e) {
            ...
            $(this).parent().find('.portlet-body form :input:visible:enabled:first').not(".skip").focus();
